<template>
  <div>
    <div class="app-container">
      ...
      <el-table :key="tableKey" v-loading="listLoading" :data="dataList" element-loading-text="数据载入中" highlight-current-row fit stripe @selection-change="handleSelectionChange" @sort-change="handleSort">
        ...
        <el-table-column align="center" label="操作" width="170" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status=='2' && authCheck(22)" class="no-margin padding-5" plain size="mini" type="warning" icon="el-icon-remove" @click="handleStatus(scope.row.id,'1')">
              禁用
            </el-button>
            <el-button v-if="scope.row.status=='1' && authCheck(22)" class="no-margin padding-5" plain size="mini" type="primary" icon="el-icon-success" @click="handleStatus(scope.row.id,'2')">
              启用
            </el-button>
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      ...
      const item = this.userGroup.find(it => {
