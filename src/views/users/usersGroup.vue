  ...
  <div>
    <div class="app-container">
      <div class="filter-container">
        ...
        <el-button v-if="authCheck(24)" v-waves class="filter-item" type="primary" icon="el-icon-circle-plus-outline" @click="handleCreate">添加</el-button>
      </div>
      <el-table :key="tableKey" v-loading="listLoading" :data="dataList" element-loading-text="数据载入中" fit stripe>
        ...
        <el-table-column min-width="100px" align="center" prop="status" label="状态" sortable>
          <template slot-scope="scope">
            <i :class="scope.row.status == '2'?'el-icon-success text-primary':'el-icon-remove text-danger'" /> {{ scope.row.status|statusFilter }}
          </template>
        </el-table-column>
        <el-table-column align="right" label="操作" width="250" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <router-link v-if="authCheck(16) && scope.row.id!=3" :to="{name:'list',params:{group:scope.row.id}}" class="el-button el-button--primary el-button--mini is-plain" tag="button">
    ...
    <groupAdd :dialog-state="dialogState" :group-id="groupId" @CB-dialogState="CB_dialogState">用户组添加编辑Dialog</groupAdd>
    <nodeTree :dialog-status="dialogStatus" :rlid="rlid" :group-title="groupTitle" @CB-dialogStatus="CB_dialogStatus">用户组权限节点Dialog</nodeTree>
