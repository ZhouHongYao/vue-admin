  ...
  <div style="overflow-x: auto;">
    <div class="app-container">
      <div class="filter-container">
        ...
        <el-button v-if="authCheck(3)" v-waves class="filter-item margin-right-10" type="primary" icon="el-icon-circle-plus-outline" @click="handleADD">添加</el-button>
      </div>
      <div v-loading="loading" class="custom-tree-container" element-loading-text="数据载入中" style="min-width:1200px;">
        <el-tree ref="nodeTree" :data="tableData" :expand-on-click-node="false" :props="defaultProps" :filter-node-method="filterNode" :allow-drop="allowDrop" :allow-drag="allowDrag" class="filter-tree" default-expand-all node-key="id" highlight-current empty-text="啊哦，您还没有添加菜单哦！" draggable @node-drop="handleDrop">
          <span slot-scope="{ node, data }" class="custom-tree-node" style="height: 42px;position: relative">
            ...
            <span style="min-width:100px;height: 42px;line-height: 42px;position:absolute;right:700px;">
              {{ data.api_name }}
            </span>
            <span style="min-width:180px;height: 42px;line-height: 42px;position:absolute;right:450px;">
              {{ data.name }}
            </span>
            <span class="hidden-md" style="width:55px;height: 42px;line-height: 42px;position:absolute;right: 380px;">
              {{ data.method || '--' }}
            </span>
            <span v-if="authCheck(14)" class="hidden-md" style="width:70px;position:absolute;right:275px;height: 42px;line-height: 42px;top: 0;">
