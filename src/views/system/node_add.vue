<template>
  <el-dialog :visible="dialogStatus" title="导航栏信息" width="580px" top="25vh" close-on-press-escape @close="cleardialog" @open="openDialog">
    <el-form ref="form" v-loading="loading" :model="form" :rules="rules" label-width="150px" element-loading-text="数据载入中">
      <el-form-item label="上级类别名称" required>
        <el-cascader v-model="form.path" :options="options" :props="props" change-on-select style="width:100%" size="medium" />
      </el-form-item>
      ...
      <el-form-item label="传值方式" prop="method">
        <el-select v-model="form.method" placeholder="请选择传值方式" filterable clearable size="medium">
          <el-option v-for="item in methodData" :key="item.title" :label="item.title" :value="item.title">
    ...
    <div v-if="!loading" slot="footer" class="dialog-footer">
      <el-button :loading="vloading" type="primary" icon="el-icon-success" @click="handleBaocun()"> 提交</el-button>
    </div>
  </el-dialog>
</template>
<script>
