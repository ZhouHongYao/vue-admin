<template>
  <span>
    <div v-if="!type || type == 'default'" :id="container?container:'SelectContainer'">
      <el-button :id="button_id?button_id:'pl_upload_button'" :size="button_size?button_size:'small'" :type="theme?theme:'info'" :plain="plain" icon="el-icon-upload">{{ button_text?button_text:'上传文件' }}</el-button>
    </div>
    <!-- 拖动上传 -->
    <div v-if="type == 'drag'" :id="container?container:'SelectContainer'">
      <div :id="button_id?button_id:'pl_upload_button'" class="el-upload el-upload--text">
        <img v-if="imageUrl" :src="imageUrl" style="width:360px;height:180px">
